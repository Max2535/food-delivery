...
- **Asynchronous Event-Driven Architecture** via RabbitMQ (Integration with Kitchen Service)

## Architecture Pipeline

Here is how the Order Service integrates with the Kitchen Service via RabbitMQ messages:

```mermaid
sequenceDiagram
    participant User
    participant OrderAPI as Order Service (API)
    participant OrderDB as Database (order_db)
    participant RabbitMQ as RabbitMQ (Broker)
    participant KitchenWorker as Kitchen Service (Worker)
    participant KitchenDB as Database (kitchen_db)

    User->>OrderAPI: POST /api/v1/orders
    OrderAPI->>OrderDB: Create new Order (status: pending)
    OrderDB-->>OrderAPI: Order ID returned
    
    OrderAPI->>RabbitMQ: Publish "order.created" Event
    RabbitMQ-->>OrderAPI: Ack
    OrderAPI-->>User: 201 Created (Order Details)
    
    RabbitMQ->>KitchenWorker: Deliver "order.created" Event
    KitchenWorker->>KitchenDB: Create new KitchenTicket (status: Received)
    KitchenDB-->>KitchenWorker: Ticket ID returned
```
